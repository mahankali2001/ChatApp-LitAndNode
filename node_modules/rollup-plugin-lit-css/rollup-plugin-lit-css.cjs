...
var rollup_plugin_lit_css_exports = {};
__export(rollup_plugin_lit_css_exports, {
  default: () => rollup_plugin_lit_css_default,
  litCss: () => litCss
});
module.exports = __toCommonJS(rollup_plugin_lit_css_exports);
...
// packages/rollup-plugin-lit-css/rollup-plugin-lit-css.ts
var import_lit_css = require("@pwrs/lit-css");
...
function litCss(options) {
  ...
  return {
    name: "lit-css",
    load(id) {
      if (filter(id)) this.addWatchFile((0, import_node_path.resolve)(id));
      return null;
    },
    async transform(css, id) {
      if (!filter(id)) return null;
      try {
        const code = await (0, import_lit_css.transform)({ css, specifier, tag, filePath: id, ...rest });
